...
def process(g):
    ...
    if g.roundType < 0:
        ...
        hero = g.me_heros[0]
        op_hero = g.op_heros[0]
        op_tower = g.op_towers[0]
        assert(isinstance(hero, RoundEntity))
        assert(isinstance(op_hero, RoundEntity))
        # =======================================  BUY  =======================================
        for item in g.itemList:
            assert(isinstance(item, Item))
            if g.me_gold > item.itemCost and item.health > 0 and item.isPotion > 0 and hero.max_health - hero.health > item.health:
                _act = 'BUY {}'.format(item.itemName)
                print(_act if debug_in else '{} {}'.format(_act, debug_in))
                return
            if g.me_gold > item.itemCost > 500 and g.me_item_num < 4:
                _act = 'BUY {}'.format(item.itemName)
                g.me_item_num += 1
                print(_act if debug_in else '{} {}'.format(_act, debug_in))
                return
        ...
        for unit in g.me_units:
            ...
            if unit.health > 75:
                ...
                break
        # Attack Op hero
        for tmx, tmy in G.get_range_pos(hero):
            if M.dist(tmx, tmy, op_hero.x, op_hero.y) < hero.attackRange and G.num_front(hero) * (_x - tmx) > 20 \
                    and M.dist(tmx, tmy, op_tower.x, op_tower.y) > op_tower.attackRange + 10:
                ext_flag = False
                for unit in g.op_units:
                    assert(isinstance(unit, RoundEntity))
                    if M.dist(tmx, tmy, unit.x, unit.y) < unit.attackRange:
                        ext_flag = True
                        break
                if not ext_flag:
                    _act = '{} {} {} {}'.format(ActType.MOVE_ATTACK.value, tmx, tmy, op_hero.unitId)
                    print(_act if debug_in else '{} {}'.format(_act, debug_in))
                    return
        # Attack Op unit
