...
class Unit:
    def __init__(self, str_line):
        ...
        self.friction = [0.2, 0.3, 0.25, 0.4, 0][self.unit_type]
...
def doof_direc(g):
    ...
    if  _dist < 3000 and dvan([g.reapers[0].x, g.reapers[0].y], [g.destroyers[0].x,g.destroyers[0].y],
            [g.reapers[0].vx, g.reapers[0].vy]) < _ang:
        _dir = get_outdirect([g.reapers[0].x, g.reapers[0].y], [g.doofs[0].x,g.doofs[0].y],step=1000)
        return doof.x+_dir[0], doof.y+_dir[1], 300
    else:
        ...
        return doof.x+dirc[i][0], doof.y+dirc[i][1], 300
...
def proc(g, scores, rages, unit_list):
    ...
    if _reaper_target:
        ...
        (tx, ty) = (0, 0)
    r_destx, r_desty, r_throttle = ppath((g.reapers[0].x, g.reapers[0].y), (tx, ty), (g.reapers[0].vx, g.reapers[0].vy),
                             g.reapers[0].mass, g.reapers[0].friction)
    # Destroyer Tanker
    _min_dis_tank = 1e10
        ...
        for w in g.list_wreck:
            if dvan((target.x,target.y), (w.x,w.y), (target.vx, target.vy)) < 3.14/4:  # right direction
                for di in range(700, 2000, 100):
                    ...
                    if dist([bx, by], [g.reapers[0].x, g.reapers[0].y]) > 3000 > \
            ...
            if dist((target.x,target.y), (w.x,w.y)) < w.radius + 100:
                for p in [(random.randint(-1000, 1000), random.randint(-1000, 1000)) for _ in range(10)]:
                    if dist([p[0], p[1]], [g.reapers[0].x, g.reapers[0].y]) > 3000 > \
                            dist((p[0], p[1]), (g.destroyers[0].x, g.destroyers[0].y)):
    ...
    if g.rages[0] > 250 and not "SKILL" in action_destroyer:
        ...
        for di in range(100, 2000, 200):
            ...
            if dist((bx, by), [g.reapers[0].x, g.reapers[0].y]) > 2000 > \
                ...
                break
    # Doof
    do_destx, do_desty, do_throttle = doof_direc(g)
    # map_dump2str(g.map)
    ...
    print("{} {} {}".format(int(do_destx), int(do_desty), int(do_throttle)))
    return debug_str
# game loop
g = G()
tout = 0
