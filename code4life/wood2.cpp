...
class R{
	...
	R(){
		...
		for(int i=0;i<sampleCount;i++){
			...
			cerr << "|" << s->sampleId << "|" << s->carriedBy << "|" << s->rank << "|" << s->expertiseGain << "|" << s->health << "|" << s->costA << "|" << s->costB << "|" << s->costC << "|" << s->costD << "|" << s->costE;
			samples.push_back(s);
		}
	}
	~R(){
...
void G::process(void)
{
	...
	if(holdId == -1){
		...
		if(me->target.compare("MOLECULES")){sprintf(action, "GOTO MOLECULES");}
		else{
