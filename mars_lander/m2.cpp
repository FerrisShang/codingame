...
class GCOM
{
	class Time
	{
		...
		void update(void){ time_rec = get_time(); }
		bool is_time_up(void){ return get_time() - time_rec > max_time; }
		bool is_time_up(long max_time){ return get_time() - time_rec > max_time; }
		long get(void){ return get_time() - time_rec; }
	};
	public:
	int round;
	string action_str;
...
class G : public GCOM
{
	...
	inline pair<float, int> is_drop_vectory(P& p, P& speed){
		...
		return action;
		/*
		if((speed.x > 0 && p.x >= target_r.x) || (speed.x < 0 && p.x <= target_l.x)) return false;
		if(speed.x > MVX || speed.x < -MVX) return false;
		if(round >= 60) cerr << "sDh " << cur_height(p) << " " << safe_drop_height(speed.y) << endl;
		int c_height = cur_height(p);
		if(c_height >= safe_drop_height(speed.y)) return false;
		int s_height = safe_limit_height(speed.y);
		if(c_height <= s_height) return false;
		return 3;
		float drop_time = (-speed.y - sqrt(speed.y*speed.y + 2*GRAVITY*-c_height)) /GRAVITY;
		if(round >= 60) cerr << "Dt " << drop_time << " | " << R2D(rotate) << endl;
		if(drop_time < abs(rotate-D2R(90)) / MAX_R) return false;
		int drop_x = p.x + speed.x * drop_time;
		if(round >= 60) cerr << "Dx " << drop_x << endl;
		if(!(target_l.x +5 < drop_x && drop_x < target_r.x -5)) return false;
		if(round >= 60) cerr << "Vectory" << endl;
		return true;
		*/
	}
	...
	inline int state_score(P& pos, P& speed, int fuel){
		...
		P target_hm(P(target_m.x, pos.y + 100));
		P target_pos(target_m.x, min(target_m.y + int(cur_height(target_hm) * 0.8 * (1-1/(1 + abs(pos.x - target_m.x)))), /*this->MAX_H*/3000));
		int l = min(pos.x, target_m.x), r = max(pos.x, target_m.x);
		FOR(i, land.size()){
			if(l < land[i].x && land[i].x < r && land[i].y > target_pos.y){
				target_pos.y = land[i].y + 350;
			}
		}
		P pos_d = target_pos - pos;
		P target_speed(max(-mvx, min( mvx, pos_d.x*mvx/mvx_len)), max(-mvy, min( mvy, pos_d.y*mvy/mvy_len)));
		P speed_d = target_speed - speed;
		return abs(pos_d.y *1) + abs(pos_d.x * 0.1) + abs(speed_d.y * 2) + abs(speed_d.x * 1);
	}

	void process(void){
		cerr << "R: " << this->round << " H: " << cur_height(pos) << endl;
		...
		P target_hm(P(target_m.x, pos.y));
		auto target_y = min(target_m.y + int(cur_height(target_hm) * 0.9 * (1-1/(1 + abs(pos.x - target_m.x)))), /*this->MAX_H*/3000);
		cerr << target_y << "  ";
		auto pos_dy = target_y - pos.y;
		cerr << max(-40, min( 40, pos_dy*40/300)) << "  ";
		cerr << endl;
		//while(!T->is_time_up());
	}
};
