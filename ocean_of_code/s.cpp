	...
	public:
		...
		inline void output(void){
			...
			FORBE(it, action.subact){
				if(*it==AC_SONAR && action.sonar >= 0){
					...
					sprintf(action_str, "%s | AC_MINE %c", action_str, action.mine);
					debug("My mine %c!\n", action.mine);
				}else if(*it==AC_TRIGGER && action.trigger.x>=0){
					sprintf(action_str, "%s | AC_TRIGGER %d %d", action_str, action.trigger.x, action.trigger.y);
		...
		class CPosSim{
			...
			static void filter_all(const G *g, class CPosSim* posSim){
				...
				FORBE(it, crac->subact){
					if(*it == AC_TORP && crac->torpedo.x >= 0){
						...
						posSim->mineCount++;
						if(psbPos->size() == 1){ posSim->minePos.push_back(psbPos->back() + c_d2p[crac->mine]); }
			...
			void update_visited_path(const vector<action_t> &act){
				...
				for(auto it=act.crbegin();it!=act.crend();it++){
					for(auto i=it->subact.rbegin();i!=it->subact.rend();i++){
						...
						}else if(*i==AC_TORP || *i==AC_SONAR || *i==AC_MINE || *i==AC_TRIGGER){ continue; }else{ break; }
					}
					if(exit_flag){ break; }
				}
				dump_grid();
			...
			void dump(void){
				...
				for(int j=0;j<min(15,(int)psbPos.size());j++,it++){ debug("(%d,%d) ", it->x, it->y); } debug("\n");
			}
			void dump_op(void){
				debug("ID:%d psbPos: %lld\n", OP(id), opPsbPos->size());
				auto it=opPsbPos->begin();
				for(int j=0;j<min(15,(int)opPsbPos->size());j++,it++){ debug("(%d,%d) ", it->x, it->y); } debug("\n");
			}
